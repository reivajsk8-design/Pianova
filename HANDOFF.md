...
**Estudio · Aprender: sonido + instrumentos + teclado pantalla completa (v0.47.0):** cuarta y final sub-entrega de **F4a (módulo Aprender, Practicar)**. El synth de la vista Aprender obtiene su **propio bus de ganancia** (`learnBus` GainNode a `LEARN_GAIN = 0.3` para evitar distorsión en melodías rápidas); `learnRoute()` en `app/learnView.ts` fija la ruta y el preset del synth por nota en vivo (sin interferir con el Estudio). **Selector de Instrumento** en la cabecera de Aprender con los 7 sonidos del synth: Piano, Piano brillante, Órgano, Cuerda, Campanas + **Guitarra** y **Flauta** nuevos (`audio/synth.ts` +2 presets `guitarra`/`flauta`). **Layout a pantalla completa:** el teclado fijo a lo ancho de la base de la vista (170px alto) y las notas que caen ocupan toda la altura restante. Cambios en `app/learnView.ts` (selector + layout + re-`resize()` en `wasHidden`), `ui/styles.css` (full-height), sin cambios de motor (Practicar/Escuchar intactos). 221 tests verdes. Pendiente: **F4c** (Acompañar/manos), **F4d** (secciones/progreso), **F4e** (Reto).
