...
**Estudio · Aprender: sonido + instrumentos + teclado pantalla completa (v0.47.0):** cuarta y final sub-entrega de **F4a (módulo Aprender, Practicar)**. El synth de la vista Aprender obtiene su **propio bus de ganancia** (`learnBus` GainNode a `LEARN_GAIN = 0.3` para evitar distorsión en melodías rápidas); `learnRoute()` en `audio/synth.ts` fija la ruta y el preset del synth por nota en vivo (sin interferir con el Estudio). **Selector de Instrumento** en la cabecera de Aprender con 6 presets: Piano/Órgano/Cuerda/Campanas/Guitarra/Flauta (`audio/synth.ts` +2 presets nuevos `guitar`/`flute`). **Layout a pantalla completa:** el teclado fijo en el ancho de la base de la vista (140px alto) y las notas que caen ocupan toda la altura restante, con scroll si es necesario. Cambios en `app/learnView.ts` (selector + layout + rescale en `wasHidden`), `ui/styles.css` (grid full-height), sin cambios de motor (Practicar/Escuchar intactos). 81 tests verdes. Pendiente: **F4c** (Acompañar/manos), **F4d** (secciones/progreso), **F4e** (Reto).

**Estudio · Aprender: teclado ancho + niveles (v0.46.0):** tercera y última sub-entrega de **F4a (módulo Aprender, Practicar)**. El teclado de la vista Aprender pasa a ser **fijo de ~3 octavas** (Do3–Do6, `keyboardRange` lo amplía si la canción tiene notas fuera); genera una geometría única que comparten la onda de notas que caen y el teclado DOM, manteniendo el alineamiento Synthesia. Nuevo selector **"Nivel"** (Fácil/Medio/Difícil/Importadas) filtra el repertorio expandido: **más melodías a mano** (`learn/song.ts` `songsByLevel`, cada `LearnSong` gana campo `level?: 1|2|3`, ausente ⇒ "Importadas") + las **canciones importadas de .mid** van al cubo "Importadas". Cambios puros en `learn/song.ts` (`songsByLevel` helper; presets actualizados + nuevos) y UI en `app/learnView.ts` (selector + filtrado dinámico). **Solo melodía** (sin acordes/manos — F4c). Motor intacto (Practicar/Escuchar siguen igual). 81 tests verdes. Pendiente: **F4c** (Acompañar/manos), **F4d** (secciones/progreso), **F4e** (Reto).
...
**Estudio · medidores de pico por canal (v0.41.0):** medidor de nivel **vertical** en cada tira del MIXER (post-fader; verde→ámbar→rojo; sube al instante, baja suave). `audio/meter.ts` `meterNorm` (puro, testeado) + `daw/channel.ts` gana un `AnalyserNode` (tap tras el `gain`, sink que no afecta al sonido) y `getLevel()`; `ui/channelstrip.ts` la barra (`.chMeterV`); `app/studioView.ts` un rAF `meterTick` que corre **solo con el MIXER abierto** (decaimiento + altura). Solo lectura; sin cambios del grafo de sonido.

**Estudio · knob mejorado + mapear synthx/EQ (v0.40.0):** menú de knob por clic derecho / long-press en TODOS los knobs (**Resetear** al valor por defecto + **Teclear valor** con `prompt`; en los mapeables además Asignar/Quitar MIDI). **Rueda** del ratón sobre el knob cambia el valor: normal 2%, **Shift** 10% (grueso), **Ctrl** 0,5% (fino) — `wheelStepFraction` (puro, testeado). Los knobs del **sinte editable** (`synthx:<canal>:<param>`) y del **editor de EQ** (`eq:thr/range/atk/rel`, banda visible) pasan a ser mapeables por MIDI (responden con su editor abierto). `ui/midiMenu.ts` → **`ui/knobMenu.ts`** (`openKnobMenu`). Solo UI, sin motor.

**Estudio · mapeo MIDI de knobs (v0.39.0):** clic derecho / long-press en un knob mapeable → **"🎹 Asignar MIDI"** → mueve un mando físico → queda mapeado a ese **CC** (con su **puerto**); **puntito** en los mapeados; girar el mando mueve el knob y aplica (absoluto 0–127→rango). Mapeable: **Vol/Pan/Human** por canal, **Swing**, y **parámetros de efectos** (canal y máster, por posición `fx:<prefix>:<slot>:<param>`). `midi/input.ts` parsea CC + `onControl`; `midi/learn.ts` (mapa `id→{cc,puerto}` en `localStorage['estudio-midimap']`, aprendizaje y enrutado, helpers puros testeados); `ui/knob.ts` `midiId` + `ui/midiMenu.ts` (menú + aviso + Esc). Mapa global; requiere MIDI conectado; efectos por posición. Sin cambios de motor.
...
**Estudio · pasada de densidad / look compacto pro (v0.33.0):** rediseño visual **solo CSS** (`ui/styles.css`) para que el Estudio deje de verse "de app móvil" y pase a un **look compacto tipo DAW/VST** (Elektron/Tempest). Apretados: tipografía (título 14px, etiquetas 10–11px, **valores numéricos en monoespaciada** vía `--pv-mono`: BPM, valores de knob, longitud, chips de canción), **esquinas a 4px** (controles/celdas) / 5px (paneles) / 6px (marco), paddings/gaps recortados, alturas menores (pads 44px, celdas de paso 26px, cabecera/onda 38px, play/rec 38px), tarjetas de efecto 138px. Sin cambios de HTML/estructura ni de motor; reversible. Basado en investigación de DAW/VST pro (Ableton/Bitwig/FabFilter/Elektron) + ideas de LMMS. Siguientes (aparte): rack de efectos compacto (estructura) y mapeo MIDI de knobs.
